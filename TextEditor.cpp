...
void TextEditor::Render(const char* aTitle, const ImVec2& aSize, bool aBorder)
{
	...
	// Push the background color from the theme
	ImGui::PushStyleColor(ImGuiCol_ChildBg, ImGui::ColorConvertU32ToFloat4(mPalette[(int)PaletteIndex::Background]));

	// Adjust spacing and style variables to match the theme
	ImGui::PushStyleVar(ImGuiStyleVar_ItemSpacing, ImVec2(0.0f, 0.0f)); // No spacing between items, adjust if necessary
	ImGui::PushStyleVar(ImGuiStyleVar_FramePadding, ImVec2(4.0f, 4.0f));  // Adjust frame padding for better consistency

	// Begin the ImGui child window with styling and flags
	if (!mIgnoreImGuiChild)
		ImGui::BeginChild(aTitle, aSize, aBorder, ImGuiWindowFlags_HorizontalScrollbar | ImGuiWindowFlags_AlwaysHorizontalScrollbar | ImGuiWindowFlags_NoMove);

	// Handle keyboard inputs if enabled
	...
	// Handle mouse inputs if enabled
	if (mHandleMouseInputs)
		HandleMouseInputs();

	// Colorize the text internally (syntax highlighting)
	ColorizeInternal();

	// Render the text editor (e.g., display the text content)
	Render();

	// Restore keyboard focus state if handled
	if (mHandleKeyboardInputs)
		ImGui::PopAllowKeyboardFocus();

	// End the ImGui child window if necessary
	if (!mIgnoreImGuiChild)
		ImGui::EndChild();

	// Restore the previous ImGui style variables and color
	ImGui::PopStyleVar(2);  // Pop both FramePadding and ItemSpacing
	ImGui::PopStyleColor(); // Pop the background color

	// Set the render flags to false after rendering is complete
	mWithinRender = false;
}
